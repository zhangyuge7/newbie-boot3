...
# JAVA应用程序的名称
APP_NAME=newbie-boot
# JAVA应用程序端口号
SERVER_PORT=8080
# jar包存放路径
JAR_PATH='/var/www/jar'
# jar包名称
JAR_NAME=newbie-admin-0.0.1-SNAPSHOT.jar
# PID 代表是PID文件
...
# java虚拟机启动参数
JAVA_OPTS="-Xms512m -Xmx512m -XX:MetaspaceSize=512m -XX:MaxMetaspaceSize=1024m -Dfile.encoding=utf-8"
...
start() {
  ...
  if [ $? -eq "1" ]; then
    ...
    # jar服务启动脚本
    nohup java $JAVA_OPTS -Xloggc:$LOG_FILE/gc/gclog.log -XX:HeapDumpPath=$LOG_FILE/gc/HeapDump.hprof -jar $JAR_PATH/$JAR_NAME -Dserver.port=$SERVER_PORT >./$LOG_FILE/run.log 2>&1 &
    echo $! > $JAR_PID
